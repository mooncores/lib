...
function Flux:Window(text, bottom,mainclr,toclose)
	...
	function Flux:Notification(desc,buttontitle)
		...
		NotificationTitle.Text = "Welcome, "..game.Players.LocalPlayer.Character.Name
	...
	function Tabs:Tab(text,ico)
		...
		function ContainerContent:Button(text, desc, callback)
			...
			Container.CanvasSize = UDim2.new(0, 0, 0, ContainerLayout.AbsoluteContentSize.Y)
            return {
                Set = function(self, newText, newDesc)
                    text = newText
                    Title.Text = newText
                    if Description and newDesc then
                        desc = newDesc
		...
		function ContainerContent:Slider(text,desc,min,max,start,callback)
            ...
            function SliderFunc:Set(newText, newDesc)
                text = newText
                Title.Text = newText
                if Description and newDesc then
                    desc = newDesc
		...
		function ContainerContent:Dropdown(text,list,callback)
            ...
            function DropFunc:Set(newText, newDesc)
                text = newText
                Title.Text = newText
                if Description and newDesc then
                    Description.Text = newDesc
		...
		function ContainerContent:Bind(text,presetbind,callback)
			local Key = (presetbind == Enum.UserInputType.MouseButton1 and "M1" or presetbind == Enum.UserInputType.MouseButton2 and "M2" or presetbind == Enum.UserInputType.MouseButton3 and "M3" or presetbind.Name)
			...
			Bind.MouseButton1Click:connect(
				function()
					...
					if inputwait.KeyCode.Name ~= "Unknown" or inputwait.UserInputType == Enum.UserInputType.MouseButton1 or inputwait.UserInputType == Enum.UserInputType.MouseButton2 or inputwait.UserInputType == Enum.UserInputType.MouseButton3 then
						BindLabel.Text = (inputwait.UserInputType == Enum.UserInputType.MouseButton1 and "M1" or inputwait.UserInputType == Enum.UserInputType.MouseButton2 and "M2" or inputwait.UserInputType == Enum.UserInputType.MouseButton3 and "M3" or inputwait.KeyCode.Name)
						Key = (inputwait.UserInputType == Enum.UserInputType.MouseButton1 and "M1" or inputwait.UserInputType == Enum.UserInputType.MouseButton2 and "M2" or inputwait.UserInputType == Enum.UserInputType.MouseButton3 and "M3" or inputwait.KeyCode.Name)
					end
					TweenService:Create(
						Title,
						TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
			...
			function(current, pressed)
				if not pressed then
					if (Key == "M1" and current.UserInputType == Enum.UserInputType.MouseButton1) or (Key == "M2" and current.UserInputType == Enum.UserInputType.MouseButton2) or (Key == "M3" and current.UserInputType == Enum.UserInputType.MouseButton3) or current.KeyCode.Name == Key then
						pcall(callback)
					end
				end
			end
            ...
            setmetatable(retPrepare, {
                __index = function(noOneGivesAFuck, index)
                    if index == "Bind" then
                        return Key
                    end
                end
        ...
        function ContainerContent:Set(newText, newDesc)
            text = newText
            TabTitle.Text = newText
            if Description and newDesc then
                Description.Text = newDesc
            end
        end
    ...
    function Tabs:Set(newText, newBottom)
        text = newText
        Title.Text = newText
        if BottomText and newBottom then
            bottom = newBottom
